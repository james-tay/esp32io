/*
   ...
   was set to 8-bit, then "duty" is 0-255, and so on.
*/
...
void f_pwm_duty(int idx, int pin, int duty)
{
  ...
  {
    snprintf(G_runtime->worker[idx].result_msg, BUF_LEN_LINE,
...
void f_pwm_cmd(int idx)
{
  ...
  if (count==1)
  {
    strncpy(G_runtime->worker[idx].result_msg,
            "pwm duty <pin> <duty>      change duty cycle on a pin\r\n"
            "pwm info                   print current PWM status\r\n"
            "pwm off <pin>              turn off PWM on a pin\r\n"
            "pwm on <pin> <freq> <res>  turn on PWM on a pin\r\n",
  ...
  if ((strcmp(cmd, "duty") == 0) && (tokens[2]) && (tokens[3]))
    f_pwm_duty(idx, atoi(tokens[2]), atoi(tokens[3])) ;
  else  
